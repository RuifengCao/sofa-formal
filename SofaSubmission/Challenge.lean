/-
# SofaSubmission/Challenge.lean — the upstream statements (with `sorry`)

Verbatim copy of `FormalConjectures/Wikipedia/MovingSofa.lean` from google-deepmind/formal-conjectures
(main @ 40e7c98, 2026-09-16; unchanged on main as of 2026-09-23 except for `formal_proof` tags),
plus the `ℝ²` notation and the two instances from `FormalConjecturesForMathlib/Geometry/2d.lean`
that it depends on.  Differences, all forced by building it without the formal-conjectures library:
* `module` / `public import FormalConjecturesUtil` / `@[expose] public section` → `import Mathlib`;
* the attributes `@[category …, AMS 49, formal_proof …]` are removed;
* `answer(volume gerversSofa)` is written `volume gerversSofa`.
Nothing else is changed; in particular every `sorry` of the upstream file is here.

This is the **challenge** for `comparator` (`comparator.json`); the proofs are in
`SofaSubmission/Solution.lean`.  Do not import this file from the library.
...
theorem sofaConstant_eq : sofaConstant = volume gerversSofa := by
  sorry
