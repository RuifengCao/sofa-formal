/-
# SofaSubmission/Defs.lean — the upstream definitions

`SofaSubmission/Challenge.lean` (the upstream statement file, verbatim) with two changes:
* the `sorry` of `GerversSofa.ABφθSpec.existsUnique` is replaced by the proof
  `Sofa.GC.spec_existsUnique` (`Sofa/GerverUnique.lean`, which imports only Mathlib); the
  constants `A, B, φ, θ` are defined from this theorem, so it has to be proved here;
* the four other theorems with `sorry` are left out (three are proved in
  `SofaSubmission/Solution.lean`; uniqueness is open).

Everything else — every definition, instance and proved lemma — is the same text in the same
order. This matters: Lean names anonymous instances after the module's root namespace (here
`SofaSubmission`, as for `Challenge.lean`) and shares auxiliary proofs (such as those of
`Nat.AtLeastTwo 2`) only within one file, so keeping one file in the upstream order makes the
declarations behind the upstream statements literally equal to those of `Challenge.lean`
(checked by `scripts/CompareUpstream.lean` and by comparator).  Every module of the proof
imports this file.

Apache-2.0, Copyright The Formal Conjectures Authors.
-/
import Mathlib
...
/-- Oriented angles make sense in 2d (copied from formal-conjectures). -/
...
/-- Two dimensional euclidean space is two-dimensional (copied from formal-conjectures). -/
